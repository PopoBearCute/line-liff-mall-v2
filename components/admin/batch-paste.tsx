...
export function BatchPaste({ onParsed }: BatchPasteProps) {
    ...
    const parseText = (text: string) => {
        ...
        lines.forEach(line => {
            ...
            // Assumed Format: 
            // Wave | Name | OrigPrice | Price | MOQ | Img | Desc | Link | WishStart | WishEnd | SaleStart | SaleEnd
            const wave = cols[0] || "1";
            const name = cols[1] || "";
            ...
            const desc = cols[6] || "";
            const link = cols[7] || "";
            const wishStart = cols[8] || "";
            const wishEnd = cols[9] || "";
            const saleStart = cols[10] || "";
            const saleEnd = cols[11] || "";

            if (name) {
                products.push({ wave, name, origPrice, price, moq, img, desc, link, wishStart, wishEnd, saleStart, saleEnd });
            }
        });
    ...
    return (
        <div className="space-y-4">
            <div className="bg-blue-50 border border-blue-200 p-4 rounded-lg text-sm text-blue-800">
                ...
                <p>請從 Excel 複製以下欄位順序 (共 12 欄)：</p>
                <div className="mt-2 font-mono text-xs bg-white p-2 border rounded overflow-x-auto whitespace-nowrap">
                    波段ID | 商品名稱 | 原價 | 團購價 | MOQ | 圖片網址 | 描述 | 商城連結 | 選品開始 | 選品結束 | 販售開始 | 販售結束
                </div>
            </div>
                        ...
                        <table className="w-full text-xs text-left whitespace-nowrap">
                            <thead className="bg-gray-100 sticky top-0">
                                <tr>
                                    ...
                                    <th className="p-2">選品時間</th>
                                    <th className="p-2">販售時間</th>
                                </tr>
                            </thead>
                            <tbody>
                                {preview.map((p, i) => (
                                    <tr key={i} className="border-t">
                                        ...
                                        <td className="p-2 text-gray-500">{p.wishStart}<br />{p.wishEnd}</td>
                                        <td className="p-2 text-gray-500">{p.saleStart}<br />{p.saleEnd}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
